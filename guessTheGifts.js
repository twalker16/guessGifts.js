...
var wishlist = [
  ...
  { name: "Toy Car", size: "medium", clatters: "a bit", weight: "medium" },
  { name: "Card Game", size: "small", clatters: "no", weight: "light" },
  { name: "Console", size: "large", clatters: "a bit", weight: "heavy" },
];
...
function guessGifts(presents, wishlist) {
  for (let i = 0; i < presents.length; i++) {
    for (let j = 0; j < wishlist.length; j++) {
      if (
        ...
        console.log(`I think present ${i + 1} is ${wishlist[j].name}`);
      }
    }
  }
}

guessGifts(presents, wishlist);
